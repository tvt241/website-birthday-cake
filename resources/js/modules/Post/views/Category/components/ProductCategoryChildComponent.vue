<template>
   <template v-for="category in categories" :key="category.id">
      <tr :class="{ 'd-none': category.depth !== 0 }"
         ...
         <td>
            ...
            <i v-if="category.children.length" class="mdi mdi-chevron-right"
               @click="handleShowChild($event, category.id)"></i>
         </td>
         <td>
      ...
      <ProductCategoryChildComponent v-if="category.children.length" @show-confirm="(id) => $emit('showConfirm', id)"
         @show-edit="(id) => $emit('showEdit', id)" :categories="category.children" />
   </template>
</template>
...
// import { useProductCategoryStore } from '~/Product/store/productCategoryStore';

// const store = useProductCategoryStore();
const props = defineProps(["categories"]);

const emits = defineEmits(["showConfirm", "showEdit"]);
...
function handleShowChild(event, id) {
